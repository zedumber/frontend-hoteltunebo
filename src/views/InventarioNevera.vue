  ...
  <div class="productos-container">
    <div class="panel-container">
      ...
      <div class="content">
        ...
        <button @click="showStockModal = true">Agregar Stock</button>
        <!-- Nuevo botón -->
    ...
    <div v-if="showStockModal" class="modal-overlay">
      <div class="modal">
        <button class="close-modal" @click="showStockModal = false">
          &times;
        </button>
        <h3>Agregar Stock</h3>
        ...
        <form @submit.prevent="addStock">
          <div class="form-group">
            <label for="stockCantidad">Cantidad a agregar:</label>
            <input
              type="number"
          ...
          <div class="form-group">
            ...
            <select v-model="selectedProductoId" id="productoId" required>
              <option
...
export default {
  ...
  data() {
    return {
      menuActive: false,
      showStockModal: false, // Nueva propiedad para el modal de agregar stock
      stockCantidad: 0, // Para guardar la cantidad de stock a agregar
      selectedProductoId: null, // Para guardar el producto seleccionado
      productos: [],
      producto: {
        nombre: "",
        descripcion: "",
  ...
  methods: {
    async addStock() {
      try {
        // Lógica para agregar stock al producto a través de la API
        const response = await axios.post(
          `/auth/pronevera/${this.selectedProductoId}/agregar-stock`,
          {
            cantidad: this.stockCantidad, // Envía la cantidad que deseas agregar
          }
        );

        // Verifica la respuesta y actualiza el stock localmente
        if (response.data.success) {
          const producto = this.productos.find(
            (p) => p.id === this.selectedProductoId
          );
          if (producto) {
            producto.cantidad += this.stockCantidad; // Actualiza la cantidad localmente
          }
          ...
          this.message = "Stock agregado exitosamente.";
          this.messageType = "success";
        } else {
          // Manejo de errores si la respuesta no es exitosa
          this.showNotificationModal = true;
          this.message = "Error al agregar stock.";
        ...
        this.message =
          error.response && error.response.data.error
            ...
            : "Hubo un problema al intentar agregar el stock.";
        this.messageType = "error";
      }
    },
    toggleMenu() {
    ...
    saveProducto() {
      const request = this.editing
        ? axios.put(
            `/auth/inventario-nevera/${this.producto.id}`,
            this.producto
          )
        : axios.post("/auth/inventario-nevera", this.producto);
    ...
    deleteProducto(id) {
      axios
        .delete(`/auth/inventario-nevera/${id}`)
        .then(() => {
          this.fetchProductos();
